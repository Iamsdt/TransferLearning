...
class TransferNetworkImg(Network.Network):

    def __init__(self, model_name='DenseNet', lr=0.003, criterion_name='NLLLoss',
                 optimizer_name='Adam', dropout_p=0.2,
                 pretrained=True, device=None, best_accuracy=0.,
                 best_accuracy_file='best_accuracy.pth',
                 chkpoint_file='chkpoint_file', head=None):
        super(TransferNetworkImg, self).__init__()

        self.model_type = 'transfer'
        self.set_transfer_model(model_name, pretrained=pretrained)
        if head is None: head = []
        if head is not None:
            self.set_model_head(model_name=model_name, head=head, optimizer_name=optimizer_name,
                                criterion_name=criterion_name,
                                lr=lr, dropout_p=dropout_p, device=device)
            self.set_model_params(criterion_name, optimizer_name, lr, dropout_p,
                                  model_name, best_accuracy, best_accuracy_file,
                                  chkpoint_file, head)

    def set_model_params(self, criterion_name, optimizer_name, lr,
                         dropout_p, model_name, best_accuracy,
                         best_accuracy_file, chkpoint_file, head):

        print('Transfer: best accuracy = {:.3f}'.format(best_accuracy))

        super(TransferNetworkImg, self).set_model_params(
            criterion_name, optimizer_name, lr, dropout_p,
            model_name, best_accuracy, best_accuracy_file, chkpoint_file)
    ...
    def set_model_head(self, model_name='DenseNet', head=None,
                       optimizer_name='Adam', criterion_name='NLLLoss',
                       lr=0.003, dropout_p=0.2, device=None):
        if head is None:
            head = {'num_inputs': 128,
                    'num_outputs': 10,
                    'layers': [],
        ...
        if model_name.lower() == 'densenet':
            if hasattr(self.model, 'classifier'):
                ...
                self.model.classifier = FC.FC(
                    num_inputs=in_features, num_outputs=head['num_outputs'],
                    layers=head['layers'], class_names=head['class_names'],
                    non_linearity=head['non_linearity'], model_type=head['model_type'],
                    model_name=head['model_name'], dropout_p=dropout_p,
                    optimizer_name=optimizer_name, lr=lr, criterion_name=criterion_name,
                    device=device)
            ...
            if hasattr(self.model, 'fc'):
                ...
                self.model.fc = FC.FC(
                    ...
                    model_type=head['model_type'], model_name=head['model_name'],
                    dropout_p=dropout_p, optimizer_name=optimizer_name, lr=lr,
                    criterion_name=self.criterion_name, device=device)

        self.head = head
        print('{}: setting head: inputs: {} hidden:{} outputs: {}'.format(
            model_name, in_features, head['layers'], head['num_outputs']))
    ...
    def _set_dropout(self, p=0.2):
        ...
        elif self.model_name.lower() == 'resnet50' or self.model_name.lower() == 'resnet':
